...
function ElectionDetailPage() {
  ...
  const fetchElection = async () => {
    try {
      ...
      // The API returns the election object directly
  ...
  const confirmVote = async () => {
    ...
    try {
      ...
      // Check for success (status 201 or response.status === 'success')
      if (response.status === "success" || response.status === 201) {
        ...
        const errorMsg =
          response.errors || response.message || "Failed to cast vote";
        alert(errorMsg);
      }
    } catch (error) {
      console.error("Error casting vote:", error);

      // Check if the error response contains vote data (successful but caught as error)
      if (error.response?.status === 201 || error.response?.status === 200) {
        // Vote was actually successful!
        const voteData = error.response?.data?.data;
        if (voteData) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-green-700">
                  You voted for: {voteResult.candidate}
                </p>
                <p className="text-xs text-green-600 mt-1">
                  Receipt: {voteResult.vote_hash}
                </p>
              </div>
            </div>
          </div>
